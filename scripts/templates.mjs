...
<html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${e(title)}</title><meta name="description" content="${e(description)}"><meta name="theme-color" content="#162239"><link rel="icon" href="${prefix}assets/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="${prefix}assets/site.css">`;
...
export function board(config, documents, categories) {
  ...
  return `${pageHead(`${config.title} · ${config.brand}`, config.description)}
...
<aside class="sidebar" aria-label="자료 탐색"><a class="brand" href="./" aria-label="${e(config.title)} 홈"><span class="brand-mark">P<span></span></span><span>${e(config.brand)}<small>RESEARCH LIBRARY</small></span></a>
  <div class="sidebar-divider"></div><div class="nav-label">WORKSPACE</div>
  ...
  <div class="sidebar-bottom"><button class="guide-button" data-open-guide>${icon('book')}자료 추가 안내${icon('chevron')}</button><div class="workspace-profile"><span class="profile-avatar">T</span><div>Team knowledge<small>함께 쌓아가는 리서치</small></div></div></div>
</aside>
<div class="workspace"><header class="topbar"><div class="breadcrumb"><span>지식 공유</span>${icon('chevron')}<strong>${e(config.title)}</strong></div><span class="topbar-label"><span class="status-dot"></span>Research library</span></header>
<main id="main"><section class="page-heading"><div><div class="eyebrow">TEAM RESEARCH</div><h1>${e(config.title)}<span class="heading-count">${documents.length}</span></h1><p>${e(config.description)}</p></div><button class="button button-white copy-link" type="button">${icon('link')}게시판 링크 복사</button></section>
